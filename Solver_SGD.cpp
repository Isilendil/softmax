...
Solver_SGD::~Solver_SGD()
{
}

void Solver_SGD::Solve(double *w, double *obj, Function_SOFTMAX *func)
{
	/*
  ...
  //int id;
	...
	double *w_test = new double[w_size];
  ...
  //compute objective
	...
	for(; iter < max_iter; iter++)
	{
      ...
      while(xi->index != -1)
			...
			{
				double beta_test = 1.0;
				primal = func->obj_primal(w);
                func->grad(w, grad_w, &grad_norm);
                double gap = delta * eta * grad_norm * grad_norm;
				double primal_test = 0;
				for(int iter_armijo = 0; iter_armijo < max_armijo ; iter_armijo++)
				{
					// test
					beta_test *= beta;
					for(int i = 0; i < w_size; i++)
					{
						w_test[i] = w[i] - eta*grad[i]*beta_test;
					}
					primal_test = func->obj_primal(w_test);

					// check
					double dif = primal - primal_test;
					double gap_real = gap * beta_test;
					if(dif >= gap_real)
                    //if( (primal-primal_test) >= gap*beta_test )
					{
						for(int i = 0; i < w_size; i++)
						{
							w[i] = w_test[i];
...
void Solver_SGD::solve_sub_problem()
{
}

double Solver_SGD::compute_obj(double *w)
{
	double obj = 0;
