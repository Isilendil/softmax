...
Solver_ADMM::Solver_ADMM(const problem *prob, int nr_class, double C, double eps, int max_iter, \
...
Solver_ADMM::~Solver_ADMM()
{
	delete [] alpha;
	//delete [] alpha_new;
	delete [] norm_list;
}
void Solver_ADMM::Solve(double *w, double *obj, Function_SOFTMAX *func)
{
	/*
		...
		for(int order = 0; order < l; order++)
		{
            ...
            for(int inner_iter = 0; inner_iter < max_inner_iter; inner_iter++)
            {
                ...
                for(int i = 0; i < nr_class; i++)
                {
                    ...
                    if(i == yi)
                    {
                        z_temp = std::min(1-alpha_temp-eps, std::max(-alpha_temp+eps, z_temp));
                    }
                    else
                    {
                        z_temp = std::min(-alpha_temp-eps, std::max(-1-alpha_temp+eps, z_temp));
                    }
...
double Solver_ADMM::compute_obj(double *w)
{
	double obj = 0;
