...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	if(model_->param.solver_type == SGD || model_->param.solver_type == EG || model_->param.solver_type == ME_DUAL || model_->param.solver_type == NEW)
	{
		...
		while(lx->index != -1)
		{
			int index = lx->index-1;
			double value = lx->value;
		  // the dimension of testing data may exceed that of training
			if(index <= n)
			{
				for(int iter_class = 0; iter_class < nr_class; iter_class++)
				{
					//dec_values[iter_class] += w[iter_class*n+index] * value;
					dec_values[iter_class] += w[index*nr_class+iter_class] * value;
				}
			}
			++lx;
		}
		...
		for(int iter_class = 0; iter_class < nr_class; iter_class++)
		{
			if(dec_values[iter_class] > dec_values[dec_max_iter])
			{
				dec_max_iter = iter_class;
			}
		}

		//return model_->label[dec_max_iter];
		return model_->label[dec_max_iter];
	}
	////////////////////////////////////////////////
