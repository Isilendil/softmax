...
Solver_ALM_FW::Solver_ALM_FW(const problem *prob, int nr_class, double C, double eps, int max_iter, \
...
Solver_ALM_FW::~Solver_ALM_FW()
{
	delete [] alpha;
	//delete [] alpha_new;
	delete [] norm_list;
}
void Solver_ALM_FW::Solve(double *w, double *obj, Function_SOFTMAX *func)
{
	/*
            ...
            for(inner_iter = 0; inner_iter < max_inner_iter; ++inner_iter, rho *= 1.1)
            {
                ...
                for(newton_iter = 0; newton_iter < max_newton_iter; ++newton_iter)
                {
                    ...
                    for(i = 0; i < nr_class; ++i)
                    {
                        ...
                        factor = 2.0 / (2 + (newton_iter+1)/3.0);
                        d[i] = factor*d_temp + (1-factor)*d[i];
                    }

                    if(G_norm <= eps_G)
                        break;
                    /*
                    G_gap = fabs( (G_max-G_min) / G_min);
                    if( fabs( (G_gap-G_gap_old)/G_gap) <= G_eps)
                    {
                        break;
                    }
                    else
...
double Solver_ALM_FW::compute_obj(double *w)
{
	double obj = 0;
