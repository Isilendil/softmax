...
#include "BLG/driver.h"
//////////////////////////////////////////////

Solver_BLG::Solver_BLG(const problem *prob, int nr_class, double C, double eps, int max_iter, \
...
Solver_BLG::~Solver_BLG()
{
	delete [] alpha;
	//delete [] alpha_new;
	delete [] norm_list;
}
void Solver_BLG::Solve(double *w, double *obj, Function_SOFTMAX *func)
{
	/*
    ...
    */
    ...
    double eps_box = 1e-30;
    double *lo = new double[nr_class];
    double *hi = new double[nr_class];
	...
	//for(; iter < 1; iter++)
	{
		...
		for(order = 0; order < l; ++order)
		{
            ...
            for(i = 0; i < nr_class; ++i)
            {
                ...
                if(i == yi)
                {
                    lo[i] = - alpha_temp + eps_box;
                    hi[i] = 1 - alpha_temp - eps_box;
                }
                else
                {
                    lo[i] = -1 - alpha_temp + eps_box;
                    hi[i] = - alpha_temp - eps_box;
                }
            }

            //int BLG_main (int n, double *x, double *lo, double *hi, double *a, double b, \
             //double *norm_list, double *q, double *alpha_list, double C, int yi);
            BLG_main (nr_class, d, lo, hi, a, 0, norm_list[id], q, alpha+(id*nr_class), C, yi);
...
double Solver_BLG::compute_obj(double *w)
{
	double obj = 0;
